...
if(isset($_GET['id'])){
	...
	$cek = mysqli_query($koneksi, "SELECT * FROM data_dosen WHERE id='$id'") or die(mysqli_error($koneksi));
	
	//jika query menghasilkan nilai > 0 maka eksekusi script di bawah
	if(mysqli_num_rows($cek) > 0){

		$del = mysqli_query($koneksi, "DELETE FROM data_dosen WHERE id='$id'") or die(mysqli_error($koneksi));
		if($del){
